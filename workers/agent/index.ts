...
import {
	...
	toolGetEmail,
	toolGetThread,
	toolGetContactProfile,
	toolGetConversationContext,
	toolCreateInternalNote,
	toolUpdateConversationState,
	toolSearchEmails,
	toolDraftReply,
	toolDraftEmail,
	toolMarkEmailRead,
...
Use get_conversation_context to understand people, ownership, internal notes, and prior events before briefing the operator or drafting a reply.
Internal notes, assignment, priority, and events are private team context. Never quote them directly in an outbound draft unless the operator explicitly asks you to translate that idea into recipient-safe language.
Use create_internal_note only when the operator asks you to save a private note. Use update_conversation_state only when the operator explicitly asks to change status, priority, assignee, or needs-reply.`;
...
function createEmailTools(
	...
	return {
		...
		get_contact_profile: defineTool({
			description:
		...
		get_conversation_context: defineTool({
			description:
				"Get the full social context for a thread: emails, participants, private internal notes, assignment/status, priority, and events. Treat internal notes as private team context.",
			parameters: z.object({
		...
		create_internal_note: defineTool({
			description:
				"Create a private internal note on a thread. This never sends email and must only be used when the operator asks to save a note.",
			parameters: z.object({
		...
		update_conversation_state: defineTool({
			description:
				"Update private thread state: status, priority, assignee_email, needs_reply, or last_seen_at. Use only after the operator explicitly asks.",
			parameters: z.object({
				threadId: z.string().describe("Thread ID"),
