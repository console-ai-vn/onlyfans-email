...
export class EmailMCP extends McpAgent<Env> {
	...
	async init() {
		...
		this.server.tool(
			...
			"Get social context for a thread: emails, participants, private internal notes, assignment/status, priority, and events. Internal notes are private team context and must not be copied into outbound email.",
			{
				mailboxId: z.string().describe("The mailbox email address"),
		...
		this.server.tool(
			...
			"Create a private internal note on a thread. This never sends email and is not visible to recipients.",
			{
				mailboxId: z.string().describe("The mailbox email address"),
				threadId: z.string().describe("The thread ID"),
				authorEmail: z.string().email().describe("Internal author email"),
		...
		this.server.tool(
			...
			"Update private thread state: status, priority, assignee_email, needs_reply, or last_seen_at. Call only after explicit operator confirmation.",
			{
				mailboxId: z.string().describe("The mailbox email address"),
				threadId: z.string().describe("The thread ID"),
