...
import { normalizeConversationStatePatch } from "./conversation-state";
import { deliverInternalEmail } from "./internal-delivery";
import { normalizeInternalNoteBody } from "./internal-notes";
...
type MailboxSocialStub = {
	getContactProfile: (emailAddress: string) => Promise<unknown>;
	getConversationParticipants: (threadId: string) => Promise<unknown[]>;
	getConversationState: (threadId: string) => Promise<unknown>;
	listInternalNotes: (threadId: string) => Promise<unknown[]>;
	listConversationEvents: (threadId: string) => Promise<unknown[]>;
	createInternalNote: (threadId: string, authorEmail: string, body: string) => Promise<unknown>;
	updateConversationState: (threadId: string, patch: unknown) => Promise<unknown>;
};
...
> {
	const stub = getMailboxStub(env, mailboxId);
	const routing = getRecipientRouting(env, { to: params.to });
	if (routing.hasExternalRecipients) {
		return { error: getInternalOnlyDeliveryError(routing.externalRecipients) };
	}
	...
	await deliverInternalEmail(env, {
		to: params.to,
		from: mailboxId,
		subject: params.subject,
		html: fullBodyHtml,
...
export async function toolGetContactProfile(
	env: Env,
	mailboxId: string,
	emailAddress: string,
) {
	const stub = getMailboxStub(env, mailboxId) as unknown as MailboxSocialStub;
	const profile = await stub.getContactProfile(emailAddress);
	if (!profile) return { error: "Contact not found", emailAddress };
	return {
		profile,
		note:
			"Contact profile combines mailbox history with private team-entered people memory fields: bio, contact description, relationship, relationship stage, tags, location, website, and AI memory.",
	};
}

export async function toolGetConversationContext(
	env: Env,
	mailboxId: string,
	threadId: string,
) {
	...
	const stub = getMailboxStub(env, mailboxId) as unknown as MailboxSocialStub;
	const thread = await toolGetThread(env, mailboxId, safeThreadId);
	const [participants, state, internalNotes, events] = await Promise.all([
		stub.getConversationParticipants(safeThreadId),
		stub.getConversationState(safeThreadId),
		stub.listInternalNotes(safeThreadId),
		stub.listConversationEvents(safeThreadId),
	]);
	...
	return {
		...
		events,
		privacy:
			"Internal notes, state, and events are private team context. Never include them verbatim in outbound email.",
	};
}

export async function toolCreateInternalNote(
	env: Env,
	mailboxId: string,
	params: { threadId: string; authorEmail: string; body: string },
) {
	const stub = getMailboxStub(env, mailboxId) as unknown as MailboxSocialStub;
	const body = normalizeInternalNoteBody(params.body);
	return stub.createInternalNote(
		requireThreadId(params.threadId),
		params.authorEmail,
		body,
	);
}

export async function toolUpdateConversationState(
	env: Env,
	mailboxId: string,
	params: { threadId: string; patch: Record<string, unknown> },
) {
	const stub = getMailboxStub(env, mailboxId) as unknown as MailboxSocialStub;
	const patch = normalizeConversationStatePatch(params.patch);
	return stub.updateConversationState(requireThreadId(params.threadId), patch);
}

function requireThreadId(threadId: string) {
	const value = threadId.trim();
	if (!value) throw new Error("threadId is required");
	return value;
}
