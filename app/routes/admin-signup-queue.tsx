...
export default function AdminSignupQueueRoute() {
	...
	const handleApprove = async (requestId: string) => {
		...
		try {
			...
			const detail = result.fullyAutomated
				? "Mailbox + Access login + email hu?ng d?n dã g?i."
				: [
						...
						!result.accessOtpAdded && !result.accessOtpSkipped
							...
							: undefined,
						!result.notificationSent ? "Chua g?i email thông báo" : undefined,
					]
						.filter(Boolean)
						.join(" · ") || "Mailbox dã t?o — ki?m tra automation";
			toast.add({
			...
			const message =
				error instanceof Error ? error.message : "Không duy?t du?c";
	...
	const handleReject = async (requestId: string) => {
		...
		try {
			...
			toast.add({ title: "Ðã t? ch?i" });
		} catch (error) {
			const message =
				error instanceof Error ? error.message : "Không t? ch?i du?c";
			toast.add({ title: message, variant: "error" });
		} finally {
			setActiveId(null);
		}
	...
	return (
		<div className="max-w-3xl px-4 py-8 md:px-8 space-y-5">
			<div className="flex flex-wrap items-center gap-2">
				<h1 className="text-xl font-semibold text-kumo-default">Duy?t dang ký</h1>
				{automationReady ? (
			...
			{isLoading ? (
				...
				<Empty
					icon={<UserPlusIcon size={48} className="text-kumo-inactive" />}
					title="Không có request ch?"
					description="Form start.onyx.com.vn s? hi?n ? dây khi có ngu?i dang ký."
				/>
			) : (
